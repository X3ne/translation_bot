...
class Queuing {
  ...
  _executeQueue(resolve, reject) {
    ...
    const queueF = this.queue[0];
    const realResolve = resolve || queueF.resolve;
    const realReject = reject || queueF.reject;
  ...
  _queueNextUp(queueF, skip = false) {
    const $this = this;
    if (skip) {
      const emitter = typeof (queueF.afterEvent.emitter) === 'function' ? queueF.afterEvent.emitter() : queueF.afterEvent.emitter;
      if (!emitter) return;
